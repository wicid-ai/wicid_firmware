    ...
    async def run_web_server(self):
        ...
        def configure(request: Request):
            ...
            try:
                ...
                self.logger.debug(f"About to check password length. Password type: {type(password)}, value: {repr(password)}")
        ...
        def validation_status(request: Request):
            ...
            try:
                ...
                return self._json_error(request, "Could not get validation status", code=500, text="Internal Server Error")
        ...
        def activate(request: Request):
            ...
            try:
                ...
                if self.validation_state != "success":
                    return self._json_error(request, "Cannot activate - validation not complete", code=400, text="Bad Request")
        ...
        def update_now(request: Request):
            ...
            try:
                ...
                if self.validation_state != "success":
                    return self._json_error(request, "Cannot install update - validation not complete", code=400, text="Bad Request")
                
                if not self.validation_result or not self.validation_result.get("update_available"):
                    return self._json_error(request, "Cannot install update - no update available", code=400, text="Bad Request")
        ...
        try:
            ...
            if self.validation_state == "validating_wifi":
                if not self.pending_ssid or not self.pending_password:
                    ...
                    self.validation_result = {
                        "error": {
                            "message": "No credentials available for validation",
                            "field": None
                        }
                    }
                    self.validation_trigger = False
